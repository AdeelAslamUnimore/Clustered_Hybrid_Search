...
void batch_process_queries(clustered_hybrid_search::PredictiveConjuctiveHNSW<float> *alg_query_aware, float *query_data, std::vector<std::vector<std::string>> &queries_meta_data, std::unordered_map<unsigned int, std::vector<std::string>> &meta_data, int dim, size_t num_threads);
int main(int argc, char const *argv[])
{
    /* code */
    int dim = 128;
     
    std::unordered_map<unsigned int, std::pair<std::vector<std::string>, int>> meta_data = reading_meta_data("/home/aa5f25/siftsmall/siftsmall_docs.csv");

    int max_elements = meta_data.size();
   
    hnswlib::L2Space space(dim);
    
    clustered_hybrid_search::PredictiveConjuctiveHNSW<float> *alg_query_aware = new clustered_hybrid_search::PredictiveConjuctiveHNSW<float>(&space, max_elements, "/home/aa5f25/Index/index.bin", meta_data); // Load existing index
  
    alg_query_aware->clustering_for_cms_and_cdf_filtering(80);
...
reading_meta_data(const std::string &file_path)
{
    ...
    while (std::getline(file, line))
    {
        ...
        std::string embedding, attribute, range_attribute_str;

        // CSV format: embedding;attribute;range_attribute
        std::getline(ss, embedding, ';');
        std::getline(ss, attribute, ';');
        std::getline(ss, range_attribute_str, ';');

        // Convert range_attribute to int
        int range_attribute = 0;
        try
        {
            range_attribute = std::stoi(attribute); // Change it to range_attribute_Str
        }
